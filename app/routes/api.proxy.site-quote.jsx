...
export const action = async ({ request }) => {
  try {
    ...
    if (file instanceof File) {
      productImageBuffer = Buffer.from(await file.arrayBuffer());
    }

    let logoImageBuffer = null;
    const logoFile = form.get("logoImage");
    if (logoFile instanceof File) {
      logoImageBuffer = Buffer.from(await logoFile.arrayBuffer());
    }

    const name = form.get("name")?.trim();
    const company = form.get("company")?.trim() || null;
    const location = form.get("location")?.trim() || null;
    const email = form.get("email")?.trim();
    const phone = form.get("phone")?.trim() || null;
    const quantity = form.get("quantity")?.trim();
    const message = form.get("message")?.trim() || null;
    const product = form.get("product")?.trim();

    if (!name || !email || !quantity || !product) {
    ...
    await prisma.quoteRequest.create({
      data: {
        ...
        logoImage: logoImageBuffer
          ? logoImageBuffer.toString("base64")
          : null,
      },
    });
